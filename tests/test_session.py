...
@pytest.mark.skipif(
    platform.node() == "carbon", reason="IPv6 tests fail on Windows right now"
)
def test_session_ipv6(mocker):
    ...
    aiobfd.session.log.debug.assert_called_once_with(
        "Setting up UDP client for %s:%s.", "::1", aiobfd.session.CONTROL_PORT
    )
    aiobfd.session.log.info.assert_called_once_with(
        "Sourcing traffic for %s:%s from %s:%s.",
        "::1",
...
def test_session_host_force_ipv4(mocker):
    ...
    session = aiobfd.session.Session(
        "localhost", "localhost", family=socket.AF_INET
    )
    aiobfd.session.log.debug.assert_called_once_with(
        "Setting up UDP client for %s:%s.",
        "localhost",
...
def test_session_host_force_ipv6(mocker):
    ...
    session = aiobfd.session.Session(
        "localhost", "localhost", family=socket.AF_INET6
    )
    aiobfd.session.log.debug.assert_called_once_with(
        "Setting up UDP client for %s:%s.",
        "localhost",
...
def test_sess_tx_interval_set_less(session, mocker):
    ...
    mocker.patch("aiobfd.session.log")
    session.desired_min_tx_interval = aiobfd.session.DESIRED_MIN_TX_INTERVAL - 1
    aiobfd.session.log.info.assert_called_once_with(
    ...
    assert (
        session._async_tx_interval == aiobfd.session.DESIRED_MIN_TX_INTERVAL - 1
    )
    assert (
        session._desired_min_tx_interval
        == aiobfd.session.DESIRED_MIN_TX_INTERVAL - 1
    )
...
def test_sess_tx_interval_set_more(session, mocker):
    ...
    aiobfd.session.log.info.assert_has_calls(
        [
            ...
            mocker.call(
                "Delaying increase in Tx Interval from %d to %d ...",
                aiobfd.session.DESIRED_MIN_TX_INTERVAL,
                aiobfd.session.DESIRED_MIN_TX_INTERVAL + 1,
            ),
        ]
    )
    assert session._async_tx_interval == aiobfd.session.DESIRED_MIN_TX_INTERVAL
    assert (
        session._final_async_tx_interval
        == aiobfd.session.DESIRED_MIN_TX_INTERVAL + 1
    )
    assert (
        session._desired_min_tx_interval
...
def test_sess_rx_interval_set_less(session, mocker):
    ...
    aiobfd.session.log.info.assert_has_calls(
        [
            ...
            mocker.call(
                "Delaying decrease in Detect Time from %d to %d ...",
                1000000 * 3,
                (1000000 - 1) * 3,
            ),
        ]
...
async def test_async_tx_pkts_mult_1(session, mocker):
    ...
    mocker.patch.object(asyncio, "sleep", new_callable=AsyncMock)
...
def test_tx_packet(session, mocker):
    ...
    aiobfd.session.log.debug.assert_called_once_with(
        "Transmitting BFD packet to %s:%s",
        "127.0.0.1",
        aiobfd.session.CONTROL_PORT,
    )
...
def test_rx_packet_auth_bit(session, valid_packet, mocker):
    ...
    assert (
        "Received packet with authentication while no "
...
def test_rx_packet_auth_sess(session, valid_packet, mocker):
    ...
    with pytest.raises(IOError) as excinfo:
        session.rx_packet(valid_packet)
    assert (
        "Received packet without authentication while "
        "authentication is configured locally." in str(excinfo.value)
    )
...
def test_rx_packet_poll(session, valid_packet, mocker):
    ...
    aiobfd.session.log.info.assert_called_once_with(
        ...
        "sending packet with Final (F) bit set.",
        "127.0.0.1",
    )
...
def test_rx_pkt_final_tx_interval(session, valid_packet, mocker):
    ...
    aiobfd.session.log.info.assert_has_calls(
        [
            mocker.call(
                "Increasing Tx Interval from %d to %d now that Poll "
                "Sequence has ended.",
                1000000,
                123000,
            )
...
def test_rx_pkt_final_detect_time(session, valid_packet, mocker):
    ...
    aiobfd.session.log.info.assert_has_calls(
        [
            mocker.call(
                "Increasing Detect Time from %d to %d now that Poll "
                "Sequence has ended.",
                1000000,
                123000,
            )
