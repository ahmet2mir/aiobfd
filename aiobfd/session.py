...
class Session:
    ...
    def required_min_rx_interval(self, value):
        ...
        log.info(
            ...
            "Poll Sequence.",
            self._required_min_rx_interval,
            value,
        )

        detect_time = self.calc_detect_time(
            self.remote_detect_mult, value, self.remote_min_tx_interval
        )
    ...
    def remote_min_tx_interval(self, value):
        if value == self._remote_min_tx_interval:
            return
        self._async_detect_time = self.calc_detect_time(
            self.remote_detect_mult, self.required_min_rx_interval, value
        )
    ...
    def remote_detect_mult(self, value):
        if value == self._remote_detect_mult:
            return
        self._async_detect_time = self.calc_detect_time(
            value, self.required_min_rx_interval, self.remote_min_tx_interval
        )
    ...
    def encode_packet(self, final=False):
        ...
        demand = (
            self.demand_mode
            and self.state == STATE_UP
            and self.remote_state == STATE_UP
        )
    ...
    async def async_tx_packets(self):
        ...
        try:
            while True:
                ...
                # being transmitted.
                if not (
                    (self.remote_discr == 0 and self.passive)
                    or (self.remote_min_rx_interval == 0)
                    or (
                        not self.poll_sequence
                        and (
                            self.remote_demand_mode == 1
                            and self.state == STATE_UP
                            and self.remote_state == STATE_UP
                        )
                    )
                ...
                if self.detect_mult == 1:
                    interval = self._async_tx_interval * random.uniform(
                        0.75, 0.90
                    )
                else:
                    interval = self._async_tx_interval * (
    ...
    def rx_packet(self, packet):  # pylint: disable=I0011,R0912,R0915
        ...
        if self.state == STATE_ADMIN_DOWN:
            log.warning(
                "Received packet from %s while in Admin Down state.",
                self.remote,
            )
            return
        ...
        while True:
            if not (self.demand_mode or self._async_detect_time is None):
                ...
                if self.state in (STATE_INIT, STATE_UP) and (
                    ...
                    self.desired_min_tx_interval = DESIRED_MIN_TX_INTERVAL
                    log.critical(
                        "Detected BFD remote %s going DOWN!", self.remote
                    )
                    log.info(
